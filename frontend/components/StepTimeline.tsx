...
const STEP_NAMES: Record<string, string> = {
  upload_received: '📥 Nhận file', file_validated: '✅ Kiểm tra file',
  text_extracted: '📖 Trích xuất nội dung', chunked: '✂️ Chia đoạn',
  translating: '🌐 Đang dịch', merged: '🔗 Ghép bản dịch',
  output_built: '📦 Tạo file đầu ra', download_ready: '⬇️ Sẵn sàng tải',
}

interface Props { steps: JobStep[]; currentStep: string }
...
export default function StepTimeline({ steps, currentStep }: Props) {
  ...
  return (
    <div className="rounded-2xl border p-5" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
      <h2 className="font-semibold mb-4 text-sm" style={{ color: 'var(--color-text)' }}>Tiến trình xử lý</h2>
          ...
          return (
            <li key={step.step_name} className="flex items-start gap-3">
              <div className="mt-0.5 flex-shrink-0">
                {isFailed ? <XCircle size={18} style={{ color: '#a12c7b' }} />
                  : isDone ? <CheckCircle2 size={18} style={{ color: '#437a22' }} />
                  : isActive ? <Loader2 size={18} className="animate-spin" style={{ color: '#01696f' }} />
              ...
              <div className="flex-1">
                ...
                {step.error_message && <p className="text-xs mt-0.5" style={{ color: '#a12c7b' }}>{step.error_message}</p>}
              </div>
            </li>
          )
        })}
